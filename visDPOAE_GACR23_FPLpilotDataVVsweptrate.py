...
def getDPgram(path,DatePat,fsamp,lat,r,Nmax):
    ...
    #plt.close('all')
    #fig,ax = plt.subplots()
    #ax.plot(np.mean(recMat1,1)+np.mean(recMat2,1)+np.mean(recMat3,1)+np.mean(recMat4,1))
    #ax.plot(np.mean(recMat1 + recMat2 + recMat3 + recMat4,1))
    ...
    #fig,ax = plt.subplots()
    #ax.plot(np.nanmean(recMat1+recMat2+recMat3+recMat4,1))
    
    #noiseM[np.abs(noiseM)>Theta] = np.nan
    ...
    return SDPOAEmwf, SDPOAENmwf, SNLmwf, SNLNmwf, fxx, tDPOAEmwf, tDPOAENmwf, tNLmwf, tNLNmwf
...
#ax1.plot(fxx2[:int(len(fxx10)//2)+1],20*np.log10(np.abs(S10N)/pREF),':')
ax1.plot(fxx2[:int(len(fxx10)//2)+1],20*np.log10(np.abs(SNL4N)/pREF),':')
