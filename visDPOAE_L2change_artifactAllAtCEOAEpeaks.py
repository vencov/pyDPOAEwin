# -*- coding: utf-8 -*-
"""
Created on Wed Nov 13 15:14:58 2024

CODE FOR ANALYSIS OF DPOAES I/O BASED ON THE FREQUENCIES ESTIMATED FROM CEOAEs
...
data = loadmat('Estimace/CEOAEfit_results_' + subjN + '.mat')  # load CEOAE estimated 

CF = 1000*data['FchosenLin'][0].flatten()  # chosen frequencies for analysis


CFidx = np.zeros_like(CF,dtype=int)
...
for i in range(len(DPioNL)):
    # Plot data lines with dynamic label
    data_line.append(ax.plot(L2list, DPioNL[i], label=r'${\it f}_2$ = ' + str(CF[i] / 1000) + ' kHz'))
    
    # Plot noise lines with matching color and style, without adding to legend
    noise_lines.append(ax.plot(L2list, NOxNL[i], color=data_line[i][0].get_color(), 
...
estimated_results = {}
for i in range(len(CF)):  # Loop through each dataset index
...
# Save the second plot as well (optional)
plt.savefig('Figures/DPgrams/ioAtCEOAEfreq' + subjN + '.png', format='png', dpi=300)  # Save the second graph
FolderName = 'Estimace/'
# Save the results to a .mat file
filename = f'estDPOAEIODatCEOAEfreq_{subjN}.mat'
savemat(FolderName+filename, estimated_results)
