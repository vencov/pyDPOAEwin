# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 16:18:57 2024
...
subjD['s055L_L2_552oct'] = ['Results/s055/sweep_rate/', '24_06_04_14_00_59_F2b']
...
def estimateDRgram(oaeDS,f2f1,f2b,f2e,octpersec,GainMic):
    ...
    # caclaulte roex and compare
    
    Nwindow = 10 # degree of roex windows
    tau01 = 0.5e-3
    tau02 = 25e-3
    w = pDP.roexwin(len(hm),Nwindow,fsamp,tau01,tau02)
    
    Nfft = 2**15
    fxx = np.arange(int(Nfft))*fsamp/(int(Nfft))
    
    SDPOAE_roex = np.fft.rfft(hm_50lin*w,int(Nfft))*np.exp(1j*2*np.pi*fxx[:int(len(fxx)//2)+1]*(nfilt/2)*1/fsamp)  # calculate spectrum for roex windowing
    SDPOAEN_roex = np.fft.rfft(hmN*w,int(Nfft))*np.exp(1j*2*np.pi*fxx[:int(len(fxx)//2)+1]*(nfilt/2)*1/fsamp)  # calculate spectrum of noise for roexwindowing
    ...
    DPgr = {'DPgr':SDPOAEmwf,'DPgrN':SDPOAENmwf,'NLgr':SNLmwf,'NLgrN':SNLNmwf,'fxx':fxx, 'DProex':SDPOAE_roex,'DProexN':SDPOAEN_roex}
    return DPgr
...
ax1.legend(['Wavelet filtering', 'Roex window', 'Noise floor (wavelet f.)', 'Noise floor (roex w.)'], bbox_to_anchor=(1.05, 1), loc='upper left')
...
plt.savefig('Wavelet_vs_roex_s055.jpg', format='jpg', dpi=300, bbox_inches='tight')
