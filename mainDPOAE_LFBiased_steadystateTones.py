...
fstep = 10
f2b = 2300  # f1 start frequency
f2e = 2300 # f2 end frequency
...
try:
    for i in range(len(f2val)):
        ...
        sigin = np.column_stack((twtones, 0*lfBias, 0*lfBias, 0*lfBias, lfBias))            
        ...
        data = {"recsig": recsig,"fsamp":fsamp,"f2f1":f2f1,"f2":f2val[i],"f1":f1valR[i],"L1":L1,"L2":L2,'micGain':micGain}  # dictionary
            

        file_name = 'ststDPOAE_' + subj_name + '_' + t[2:] + '_' + 'F2' + '_' + str(f2val[i]) + 'F1' + '_' + str(f1valR[i]) + 'L1' + '_' + str(L1) + 'dB' + '_' + 'L2' + '_' + str(L2) + 'dB' + '_' + 'f2f1' + '_' + str(int(f2f1 * 100)) + '_Lbias' + str(LBias) + 'dB_' + ear_t
